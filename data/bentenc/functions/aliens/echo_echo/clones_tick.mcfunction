...
execute as @a[scores={bentenc.alien=32},distance=..50] unless score @e[type=wolf,tag=bentenc.echo_echo.clone,distance=0,limit=1] glcore.id = @s glcore.id run kill @e[type=wolf,tag=bentenc.echo_echo.clone,distance=0,limit=1]
