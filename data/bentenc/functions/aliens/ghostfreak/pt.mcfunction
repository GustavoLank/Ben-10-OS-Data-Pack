execute as @e[distance=..5,type=#glcore:living,tag=bentenc.body_possessed,sort=nearest,nbt=!{ignited:1b}] if score @s bentenc.possesID = @p glcore.id rotated ~ 0 run function bentenc:aliens/ghostfreak/pt_
